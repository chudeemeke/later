...
export class HelpGenerator {
  ...
  static main(version: string): string {
    ...
    lines.push(
      Colors.bold(`later v${version}`) +
        Colors.dim(" - Deferred decision management CLI"),
    );
    lines.push("");
    lines.push(Colors.bold("USAGE:"));
    lines.push(
      `  ${Colors.dim("$")} later ${Colors.dim("<command> [options]")}`,
    );
    lines.push("");
    lines.push(Colors.bold("COMMANDS:"));
    lines.push(
      `  ${Colors.bold("capture")}        Capture a new deferred decision`,
    );
    lines.push(
      `  ${Colors.bold("list")}           List deferred items with filtering`,
    );
    lines.push(
      `  ${Colors.bold("show")}           Show full details of a specific item`,
    );
    lines.push(
      `  ${Colors.bold("do")}             Mark item as in-progress and get todo guidance`,
    );
    lines.push(`  ${Colors.bold("update")}         Update an existing item`);
    lines.push(
      `  ${Colors.bold("delete")}         Delete an item (soft or hard)`,
    );
    lines.push(
      `  ${Colors.bold("bulk-update")}    Update multiple items at once`,
    );
    lines.push(
      `  ${Colors.bold("bulk-delete")}    Delete multiple items at once`,
    );
    lines.push(
      `  ${Colors.bold("search")}         Full-text search across items`,
    );
    lines.push("");
    lines.push(Colors.bold("GLOBAL OPTIONS:"));
    lines.push(`  ${Colors.dim("--help, -h")}         Show help information`);
    lines.push(
      `  ${Colors.dim("--version, -v")}      Show version information`,
    );
    ...
    lines.push(
      `  ${Colors.dim("$")} later capture "Should we use PostgreSQL or MongoDB?"`,
    );
    lines.push(
      `  ${Colors.dim("$")} later list --status pending --priority high`,
    );
    ...
    lines.push(
      Colors.dim(
        "Run 'later <command> --help' for detailed information on a specific command.",
      ),
    );
  ...
  static subcommand(name: string): string {
    ...
    // Header
    lines.push(
      Colors.bold(`later ${name}`) + Colors.dim(` - ${schema.description}`),
    );
    lines.push("");
  ...
  private static generateUsage(name: string, schema: CommandSchema): string {
    ...
    if (schema.requiredPositional) {
      if (schema.requiredPositional === 1) {
        ...
        // Multiple positional args (not currently used by any command)
  ...
  private static getPositionalDescriptions(command: string): string[] {
    const descriptions: Record<string, string[]> = {
      capture: [
        "<decision>          Decision text to defer (required, max 500 chars)",
      ],
      show: ["<id>                Item ID to show (required, number)"],
      do: [
        "<id>                Item ID to start working on (required, number)",
      ],
  ...
  private static generateFlagHelp(flags: Record<string, FlagSchema>): string[] {
    ...
    for (const [name, schema] of Object.entries(flags)) {
      ...
      const originalLength = schema.short
        ? `--${name}, -${schema.short}`.length +
          (schema.type !== "boolean"
            ? this.getValuePlaceholder(schema).length + 1
            : 0)
        : `--${name}`.length +
          (schema.type !== "boolean"
            ? this.getValuePlaceholder(schema).length + 1
      ...
      // Add required marker (not currently used by any command flag)
      if (schema.required) {
  ...
  private static getExamples(command: string): string[] {
    const examples: Record<string, string[]> = {
      ...
      show: ["later show 5", "later show 1"],
      do: ["later do 3", "later do 1"],
      update: [
      ...
      delete: ["later delete 5", "later delete 3 --hard"],
      "bulk-update": [
