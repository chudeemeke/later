...
describe('bulk-update command handler', () => {
  ...
  it('should update multiple items with priority', async () => {
    ...
    const exitCode = await handleBulkUpdate(parsed, mockClient);
  ...
  it('should handle all failures', async () => {
    ...
    await expect(async () => {
      await handleBulkUpdate(parsed, mockClient);
    }).rejects.toThrow();
  });
  ...
  it('should handle decision update', async () => {
    ...
    const parsed = createParsedArgs(['5'], { decision: 'Updated decision text' });
    const exitCode = await handleBulkUpdate(parsed, mockClient);
