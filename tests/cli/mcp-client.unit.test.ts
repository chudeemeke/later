...
describe('McpClient Unit Tests', () => {
  ...
  describe('callTool', () => {
    it('should throw error when client is closed', async () => {
      ...
      await expect(client.callTool('later_list', {})).rejects.toThrow('MCP client has been closed');
    });
    ...
    it('should handle result with isError flag', async () => {
      ...
      const client = new McpClient(undefined, undefined, 5000, false);
      await expect(client.callTool('later_show', { id: 1 })).rejects.toThrow('Tool error occurred');
    });
    ...
    it('should handle isError with empty content', async () => {
      ...
      const client = new McpClient(undefined, undefined, 5000, false);
      await expect(client.callTool('later_show', { id: 1 })).rejects.toThrow('Tool execution failed');
    });
    ...
    it('should handle parsed result with success=false', async () => {
      ...
      const client = new McpClient(undefined, undefined, 5000, false);
      await expect(client.callTool('later_show', { id: 999 })).rejects.toThrow('Item not found');
    });
    ...
    it('should timeout when operation takes too long', async () => {
      ...
      await expect(client.callTool('later_list', {})).rejects.toThrow(/timeout/i);
    });
    ...
    it('should cleanup after failed call', async () => {
      ...
      await expect(client.callTool('later_list', {})).rejects.toThrow('Connection failed');
      expect(mockTransportClose).toHaveBeenCalled();
    });
  });
  ...
  describe('getServerVersion', () => {
    ...
    it('should throw error when server is unreachable', async () => {
      ...
      await expect(client.getServerVersion()).rejects.toThrow('Unable to connect to MCP server');
    });
